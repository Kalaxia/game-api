...
func AssignShipToFleet(w http.ResponseWriter, r *http.Request) {
	...
	ship := shipManager.GetShip(uint16(id), player.Id)
	fleet := manager.GetFleet(uint16(id), player.Id)
...
func RemoveShipFormFleet(w http.ResponseWriter, r *http.Request){
	/**
	 * treat the http request to remove a ship form a fleet into an hangard
	 *
	 *
	 */
