...
func (c *Client) processMessage(message []byte) {
	...
	if err := json.Unmarshal(message, &data); err != nil {
		panic(exception.NewException("Cannot unmarshal WS message", nil))
	}
	switch (data["action"]) {
		case "authenticate": 
			token := security.AuthenticateJwt(data["token"].(string))
			c.player = manager.GetPlayer(security.GetJwtPlayerId(token.Claims.(jwt.MapClaims)), true)
			if c.player == nil {
